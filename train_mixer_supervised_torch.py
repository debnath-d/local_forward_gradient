...
"""A basic CIFAR example using Numpy and torch.
...
def update_local_stopgrad_overlap_backprop(params, batch, key):
    ...
    def local_loss_(params, batch):
        final_loss1_, local_loss1_, (states, logs) = loss(
            params,
            batch,
            noise=None,
            stop_gradient=True,
        ...
        final_loss2_, local_loss2_, (_, logs) = loss(
            params,
            batch,
            noise=None,
            stop_gradient=True,
...
def update_global_forward_grad_weights(params, batch, key):
    ...
    def local_loss(params):
        loss_, local_loss_, (states, logs) = loss(
            params,
            batch,
            stop_gradient=False,
            readout=False,
...
def update_global_forward_grad_activations(params, batch, key):
    ...
    num_groups = FLAGS.num_groups
    group_ratio = (
        [int(g) for g in FLAGS.group_ratio.split(",")]
        if FLAGS.group_ratio is not None
        else [1] * FLAGS.num_blocks
    )
    downsample = (
        [int(d) for d in FLAGS.downsample.split(",")]
        if FLAGS.downsample is not None
        else [1] * FLAGS.num_blocks
    )
    channel_ratio = (
        [int(c) for c in FLAGS.channel_ratio.split(",")]
        if FLAGS.channel_ratio is not None
        else [1] * FLAGS.num_blocks
    )
...
def update_forward_grad_activations_vmap(params, batch, key):
    ...
    grads_now = [
        (jnp.mean(grad_w, axis=0), jnp.mean(grad_b, axis=0))
    ...
    for npass in range(num_passes):
        ...
        for i, ((weight, bias), dz) in enumerate(zip(main_params, noise)):
            ...
            if blk > 0 and layer == 0 and conv:
                ...
                x_pad = jnp.pad(
                    pre_act,
                    [(0, 0), (psize, psize), (psize, psize), (0, 0)],
                    mode="constant",
                    constant_values=0.0,
                )
...
def update_direct_feedback_alignment(params, batch, key):
    ...
    def loss_fn(params, batch):
        final_loss_, local_loss_, (states, logs) = loss_dfa(
...
def update_local_feedback_alignment(params, batch, key):
    ...
    if FLAGS.same_head:
        ...
        for i in range(NL, len(grads_now) - 1):
            grads_now[i] = (
                grads_now[i][0] / float(G_),
                grads_now[i][1] / float(G_),
    ...
    if avgpool_token:
        ...
        for i in range(NL, len(grads_now) - 1):
            grads_now[i] = (
                grads_now[i][0] / float(P_),
                grads_now[i][1] / float(P_),
