...
flags.DEFINE_string("init_scheme", "kaiming", "kaiming or lecun or constant or list")
...
def mlp_block(inputs, params, noise=None, name="", mask=None, stop_every_layer=False):
    # Token mixing.
...
def _simclr_loss(x, y, temp, labels_idx):
    ...
    logits_aa = jnp.dot(normed_x, jnp.transpose(x_all)) / temp - masks * LARGE_NUM
    logits_ab = jnp.dot(normed_x, jnp.transpose(y_all)) / temp
    logits_ba = jnp.dot(normed_y, jnp.transpose(x_all)) / temp
    logits_bb = jnp.dot(normed_y, jnp.transpose(y_all)) / temp - masks * LARGE_NUM
...
def _simclr_loss_sg(x, y, temp, labels_idx):
    ...
    # [N, D] x [M, D] -> [N, M]
    logits_aa = jnp.dot(normed_x, jnp.transpose(x_all_sg)) / temp - masks * LARGE_NUM
    logits_ab = jnp.dot(normed_x, jnp.transpose(y_all_sg)) / temp
    logits_ba = jnp.dot(normed_y, jnp.transpose(x_all_sg)) / temp
    logits_bb = jnp.dot(normed_y, jnp.transpose(y_all_sg)) / temp - masks * LARGE_NUM
    logits_a = jnp.concatenate([logits_ab, logits_aa], axis=1)
    logits_b = jnp.concatenate([logits_ba, logits_bb], axis=1)
    loss_a = softmax_cross_entropy(logits_a, labels, reduction="none")
    loss_b = softmax_cross_entropy(logits_b, labels, reduction="none")
...
def _local_simclr_loss_fused(x, y, w, b, temp, labels_idx, custom_forward=False):
...
def local_simclr_loss_sg(x, y, w, b, temp, labels_idx, custom_forward=False):
    loss = _local_simclr_loss_fused(
        x, y, w, b, temp, labels_idx, custom_forward=custom_forward
    )
    return loss
...
def loss_dfa(params, batch, noise=None, key=None):
    ...
    batch = augmentations.postprocess(
        batch, subkey, add_gaussian_blur=FLAGS.gaussian_blur
    )
    view1, view2, targets, labels_idx = (
        batch["view1"],
        batch["view2"],
        batch["labels"],
        batch["labels_idx"],
    ...
    if FLAGS.simclr_sg:
        loss_ssl = jnp.mean(simclr_loss_sg(proj1, proj2, FLAGS.temp, labels_idx)) * 2.0
    else:
    ...
    for j, (weights, bias, weights_b, bias_b) in enumerate(params[:NL]):
        ...
        if FLAGS.simclr_sg:
            local_loss_bw_ = (
                jnp.mean(simclr_loss_sg(x_proj1, x_proj2, FLAGS.temp, labels_idx)) * 2.0
            )
        else:
...
def local_loss(
    block_idx, block_params, batch, key=None, noise=None, custom_forward=False
):
    if block_idx == 0:
        key, subkey = jax.random.split(key)
        batch = augmentations.postprocess(
        ...
        view1, view2 = batch["view1"], batch["view2"]
    B = view1.shape[0]
    x1, (states1, logs) = run_block(
        block_idx, FLAGS.num_groups, view1, block_params, noise
    )
    x2, (states2, _) = run_block(
        block_idx, FLAGS.num_groups, view2, block_params, noise
    )
    local_loss_ = local_simclr_loss_sg(
        x1,
        x2,
        block_params[-1][0],
        block_params[-1][1],
...
def update_backprop(params, batch, key):
    ...
    if FLAGS.optimizer == "sgd" and wd > 0.0:
        grads_now = [
            (gw + wd * w, gb) for (gw, gb), (w, b) in zip(grads_now[:-1], params[:-1])
        ] + grads_now[-1:]
...
def update_local_backprop(params, batch, key):
    ...
    def local_loss_(params, batch):
        final_loss_, local_loss_, (states1, states2, logs) = loss(
            params, batch, noise=None, stop_gradient=False, readout=True, key=key
        )
...
def update_local_stopgrad_overlap_backprop(params, batch, key):
    ...
    def local_loss_(params, batch):
        final_loss1_, local_loss1_, (states1, states2, logs) = loss(
            params,
            batch,
            noise=None,
            stop_gradient=True,
        ...
        final_loss2_, local_loss2_, (_, _, logs) = loss(
            params,
            batch,
            noise=None,
            stop_gradient=True,
        ...
        final_loss_ = (
            final_loss1_
            + final_loss2_
            + jnp.sum(jnp.stack([jnp.sum(l) for l in local_loss1_]))
            + jnp.sum(jnp.stack([jnp.sum(l) for l in local_loss2_]))
        )
        return final_loss_, (states1, states2, logs)
...
def update_backprop_readout(params, batch, key):
    ...
    def loss_fn(readout_params, batch):
        ...
        final_loss_, (states, logs) = classif_loss(
            full_params, batch, noise=None, stop_gradient=False, key=key
        )
...
def get_local_stopgrad_greedy_backprop_fn(block_idx):
    def update_local_stopgrad_greedy_backprop(block_params, batch, key):
        ...
        def local_loss_(block_params, batch):
            loss_, (x1, x2), (states1, states2, logs) = local_loss(
                block_idx,
                block_params,
                batch,
                key=key,
        ...
        grads_now, (x1, x2, _, _, logs) = grad(local_loss_, has_aux=True)(
            block_params, batch
        )

        if FLAGS.optimizer == "sgd" and wd > 0.0:
            grads_now = [
                (gw + wd * w, gb) for (gw, gb), (w, b) in zip(grads_now, block_params)
            ]
...
def update_forward_grad_weights(params, batch, key):
    ...
    def local_loss(params):
        _, local_loss_, (states1, states2, logs) = loss(
            params,
            batch,
    ...
    for npass in range(num_passes):
        ...
        # [L,B,P]
        _, g, (states1, states2, logs) = jax.jvp(
            local_loss, [params], [noise], has_aux=True
        )
...
def update_global_forward_grad_weights(params, batch, key):
    ...
    def local_loss(params):
        loss_, local_loss_, (states1, states2, logs) = loss(
            params,
            batch,
            stop_gradient=False,
            readout=False,
...
def update_forward_grad_activations(params, batch, key):
    ...
    for npass in range(num_passes):
        ...
        # [L,B,P]
        _, g, (states1, states2, logs) = jax.jvp(
            local_loss_, [zeros], [noise], has_aux=True
        )
        ...
        for i, ((weight, bias), dz) in enumerate(zip(main_params, noise)):
            ...
            if FLAGS.norm_grad and FLAGS.post_linear_ln:
                ...
                if blk > 0 and layer == 0:
                    grad_z1 = jnp.reshape(
                        ln_grad1(
                            prenorm_act1, jnp.reshape(grad_z1, prenorm_act1.shape)
                        ),
                        dz1.shape,
                    )
                    grad_z2 = jnp.reshape(
                        ln_grad1(
                            prenorm_act2, jnp.reshape(grad_z2, prenorm_act2.shape)
                        ),
                        dz2.shape,
                    )
                else:
...
def update_global_forward_grad_activations(params, batch, key):
    ...
    for npass in range(num_passes):
        ...
        # [L,B,P]
        _, g, (states1, states2, logs) = jax.jvp(
            loss_fn, [zeros], [noise], has_aux=True
        )
...
def get_forward_grad_activations_fn(block_idx):
    def _update_forward_grad_activations(block_params, batch, key):
        ...
        md = get_dataset_metadata(FLAGS.dataset)
        grads_now = [
            (jnp.zeros_like(weight), jnp.zeros_like(bias))
        ...
        for npass in range(num_passes):
            ...
            # [L,B,P]
            _, g, (x1, x2, states1, states2, logs) = jax.jvp(
                local_loss_, [zeros], [noise], has_aux=True
            )
...
def update_direct_feedback_alignment(params, batch, key):
    ...
    def loss_fn(params, batch):
        final_loss_, local_loss_, (states1, states2, logs) = loss_dfa(
            params, batch, noise=None, key=key
        )
        final_loss_ = final_loss_ + jnp.sum(
            jnp.stack([jnp.sum(l) for l in local_loss_])
        )
    ...
    grads_now, (states1, states2, logs) = grad(loss_fn, has_aux=True)(params, batch)

    # Clear gradient for backward weights.
    grads_now = [(gw, gb, 0.0 * gwb, 0.0 * gbb) for (gw, gb, gwb, gbb) in grads_now]
    if FLAGS.freeze_backbone:
        grads_now = [
            (0.0 * gw, 0.0 * gb, 0.0 * gwb, 0.0 * gbb)
            for (gw, gb, gwb, gbb) in grads_now[:-1]
        ] + [grads_now[-1]]

    if FLAGS.optimizer == "sgd" and wd > 0.0:
        grads_now = [
            (gw + wd * w, gb, gwb, gbb)
            for (gw, gb, gwb, gbb), (w, b, wb, bb) in zip(grads_now, params)
        ]
    return grads_now, logs
...
def update_local_feedback_alignment(params, batch, key):
    ...
    NL = get_num_layers(NBLK)
    grads_now, (states1, states2, logs) = grad(local_loss, has_aux=True)(params, batch)
    ...
    if FLAGS.same_head:
        for i in range(NL, len(grads_now) - 1):
            grads_now[i] = (
                grads_now[i][0] / float(G),
                grads_now[i][1] / float(G),
    ...
    if avgpool_token:
        for i in range(NL, len(grads_now) - 1):
            grads_now[i] = (
                grads_now[i][0] / float(P),
                grads_now[i][1] / float(P),
...
def update_local_stopgrad_feedback_alignment(params, batch, key):
    ...
    def local_loss(params, batch):
        final_loss_, local_loss_, (states1, states2, logs) = loss(
            params, batch, noise=None, stop_gradient=True, readout=True, key=key
        )
        final_loss_ = final_loss_ + jnp.sum(
            jnp.stack([jnp.sum(l) for l in local_loss_])
        )
...
def _random_crop(image):
    ...
    target_area = tf.random.uniform([], FLAGS.area_lb, 1.0, dtype=tf.float32) * area
...
def _center_crop(image, img_size=224, crop_padding=32):
    ...
    image = image[
        offset_height : offset_height + img_size,
...
def get_dataset(split, seed=0):
    ...
    def index(batch):
        batch["labels_idx"] = (
            tf.range(FLAGS.batch_size) + jax.process_index() * FLAGS.batch_size
        )
        if is_parallel:
            for k in batch:
                batch[k] = tf.reshape(
...
def get_optimizer(lr, num_epochs, num_ex, batch_size, schedule, weight_decay_mask=None):
    ...
    if optimizer_name in ["sgd"]:
        ...
        optimizer = optax.adamw(
            scheduler, weight_decay=FLAGS.wd, mask=weight_decay_mask
        )
...
def run_exp(
    mode, lr, train_loader, train_eval_loader, test_loader, num_epochs, log_dir
):
    if mode == "backprop":
    ...
    if ckpt_fn_list is not None:
        ...
        if mode in [
            ...
            params = [(p[0], p[1], pbw[0], pbw[1]) for p, pbw in zip(params, params_bw)]
        key = jax.random.PRNGKey(0)
    ...
    if mode in [
        ...
        wd_mask = (
            [(True, False, False)] * NFIRST
            + [(True, False, False)] * (FLAGS.num_blocks - 1) * NLAYER
            + [(False, False, False)] * (FLAGS.num_blocks + 2)
        )
    elif mode in ["direct_feedback_alignment"]:
        wd_mask = (
            [(True, False, False, False)] * NFIRST
            + [(True, False, False, False)] * (FLAGS.num_blocks - 1) * NLAYER
            + [(False, False, False, False)] * (FLAGS.num_blocks + 2)
        )
    else:
        wd_mask = (
            [(True, False)] * NFIRST
            + [(True, False)] * (FLAGS.num_blocks - 1) * NLAYER
            + [(False, False)] * (FLAGS.num_blocks + 2)
        )
    if FLAGS.linear_scale:
        scaling = FLAGS.batch_size * jax.process_count() / 256
        lr_scale = lr * scaling
    ...
    if is_parallel:
        ...
        if epoch_start == 0:
            ...
            readout_opt_state = jax.pmap(readout_optimizer.init, axis_name="i")(
                [params[-1]]
            )
        else:
            ...
            readout_opt_state = jax.pmap(lambda i: readout_opt_state, axis_name="i")(
                jnp.zeros([num_parallel], dtype=jnp.int32)
            )
    else:
